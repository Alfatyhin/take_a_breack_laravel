...
class SendpulseService
{
    ...
    protected function sendRequest($path, $method = 'GET', $data = array(), $useToken = true)
    {
        ...
        switch ($method) {
            case 'POST':
                $response = Http::withToken($this->token)->acceptJson()->post($url, $data);
                break;
            case 'PUT':
                $response = Http::withToken($this->token)->acceptJson()->put($url, $data);
                break;
            case 'DELETE':
                $response = Http::withToken($this->token)->acceptJson()->delete($url, $data);
                break;
            default:
                $response = Http::withToken($this->token)->acceptJson()->get($url);
        }



        if ($response->status() === 401 && $this->refreshToken === 0) {
            ++$this->refreshToken;
            $this->getToken();
            $retval = $this->sendRequest($path, $method, $data);
        } else {
            $retval = new stdClass();
            $retval->data = json_decode($response->body());
            $retval->http_code = $response->status();
    ...
    public function getWhatsapp($comand)
    {
        $requestResult = $this->sendRequest('whatsapp/'.$comand);

        return $this->handleResult($requestResult);
    }

    public function sendWhatsapp($comand, $data)
    {
        $requestResult = $this->sendRequest("whatsapp/$comand", 'POST', $data);
